...
```bash
bash /absolute/path/to/skills/librarian/references/checkout.sh <repo> --path-only
```

Examples:

```bash
bash /absolute/path/to/skills/librarian/references/checkout.sh mitsuhiko/minijinja --path-only
bash /absolute/path/to/skills/librarian/references/checkout.sh github.com/mitsuhiko/minijinja --path-only
bash /absolute/path/to/skills/librarian/references/checkout.sh https://github.com/mitsuhiko/minijinja --path-only
```
...
```bash
bash /absolute/path/to/skills/librarian/references/checkout.sh <repo> --force-update --path-only
```

## Recommended workflow

1. Resolve repository path via `references/checkout.sh --path-only`.
2. Use that path for searching, reading, and analysis.
3. On later references to the same repo, call `references/checkout.sh` again; it will find and update the cached checkout.
